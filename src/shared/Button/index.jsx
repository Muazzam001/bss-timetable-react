...
const Button = ({className, title, type, onClick, color, icon, rounded, hover, disabled}) => {
    ...
    return (
        <button
            className={`group flex items-center justify-center gap-x-2 text-white font-primary font-medium text-base p-2 border border-transparent shadow-sm ${className} ${rounded ? "rounded-full" : "rounded-lg"} ${!disabled ? hover === false ? null : buttonColor : ""} ${disabled ? "cursor-not-allowed bg-blue-dark2/40 text-gray-medium" : "cursor-pointer"}`}
